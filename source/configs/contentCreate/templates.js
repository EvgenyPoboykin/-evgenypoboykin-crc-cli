...
const componentTsWithoutLogic = (name, style) => {
    return [`import React, { memo } from 'react';`, styleImport(style), '', `const ${name} : React.FC = memo(() => {`, `    return ${styleTag(style, name)};`, `});`, `export default ${name};`].join('\n');
};
const componentTsWithLogic = (name, style) => {
    return [`import React, { memo } from 'react';`, `import ${name}_Logic from './logic';`, styleImport(style), '', `const ${name} : React.FC = memo(() => {`, `    const [] = ${name}_Logic();`, `    return ${styleTag(style, name)};`, `});`, `export default ${name};`].join('\n');
};
...
module.exports = { index, componentJsWithoutLogic, componentJsWithLogic, componentTsWithoutLogic, componentTsWithLogic, styleFile, logicCode };
